...
import { ArrowRight, CalendarDays, Flame, Plus, Sparkles, Swords, Users2 } from "lucide-react";
...
export default async function DashboardPage() {
  ...
  const skills: [string, number | null | undefined][] = [
    ...
    ["Listening", data.stats?.listening_score], ["Spelling", data.stats?.spelling_score]
  ];
    ...
    <>
      ...
      <main className="page-shell"><div className="app-container">
        ...
        <div className="dashboard-grid"><div className="dash-main">
          ...
          <section className="surface dash-panel"><div className="panel-heading"><h2>Recent matches</h2></div>{data.matches.length ? <div className="match-list">{data.matches.map((match) => <div className="match-row" key={match.id}><div><strong>{match.title}</strong><p>{match.opponentName ? `vs ${match.opponentName}, ` : ""}{new Intl.DateTimeFormat("en", { dateStyle: "medium" }).format(new Date(match.createdAt))}</p></div><strong className="mono">{match.score} - {match.opponentScore ?? "-"}</strong><span className="outcome">{match.winnerId ? match.winnerId === user.id ? "Won" : "Finished" : "Active"}</span></div>)}</div> : <div className="empty-inline">Completed matches will appear here.</div>}</section>
        </div><aside className="dash-side">
          <section className="surface dash-panel"><div className="panel-heading"><h2>Start learning</h2><Swords size={19} className="text-accent" /></div><div className="quick-actions"><Link className="button button-primary" href="/create-room"><Users2 size={17} /> Create</Link><Link className="button button-secondary" href="/join-room">Join</Link></div></section>
