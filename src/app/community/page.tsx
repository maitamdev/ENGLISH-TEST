...
export default async function CommunityPage() {
  ...
  const [profileResult, friendshipResult, inviteResult, ratingResult, membershipsResult, blocksResult, reportsResult] = await Promise.all([
    ...
    admin.from("user_blocks").select("blocked_id, reason, created_at").eq("blocker_id", user.id).order("created_at", { ascending: false }),
    admin.from("user_reports").select("id, reported_user_id, category, status, created_at").eq("reporter_id", user.id).order("created_at", { ascending: false }).limit(30)
  ]);
  const peerIds = [...new Set((friendshipResult.data ?? []).map((item) => item.requester_id === user.id ? item.addressee_id : item.requester_id).concat((inviteResult.data ?? []).map((item) => item.sender_id), (blocksResult.data ?? []).map((item) => item.blocked_id)))];
  ...
  const blocks = (blocksResult.data ?? []).flatMap((item) => { const peer = peers.find((value) => value.id === item.blocked_id); return peer ? [{ person: { id: peer.id, displayName: peer.display_name, username: peer.username, avatarUrl: peer.avatar_url }, reason: item.reason, createdAt: item.created_at }] : []; });
  return <><SiteHeader app displayName={profileResult.data?.display_name} /><main className="page-shell"><div className="app-container"><header className="page-header"><div><h1 className="page-title">Học cùng đúng người.</h1><p className="page-lead">Bạn bè, lời mời, rating và lựa chọn an toàn đều đến từ hoạt động thật trong Supabase.</p></div></header><CommunityHub friendships={friendships} invites={invites} ratings={(ratingResult.data ?? []).map((item) => ({ skill: item.skill, rating: Number(item.rating), matchCount: item.match_count, wins: item.wins, losses: item.losses, draws: item.draws }))} openRoom={openRoom ? { id: openRoom.id, code: openRoom.code } : null} blocks={blocks} reports={(reportsResult.data ?? []).map((item) => ({ id: item.id, userId: item.reported_user_id, category: item.category, status: item.status, createdAt: item.created_at }))} /></div></main></>;
}
