...
type SpeakingAssessment = { overall?: number; content?: number; pronunciation?: number; fluency?: number; grammar?: number; vocabulary?: number; task?: number; coherence?: number; feedbackVi?: string; strengths?: string[]; improvements?: string[]; intelligibility?: number; segmental?: number; wordStress?: number; rhythm?: number; intonation?: number; practiceDrills?: string[]; phonemeFeedback?: { phoneme: string; issue: string; example: string }[] };
type ReviewSubmission = { userId: string; answer: string; correct: boolean; timedOut: boolean; matchedAnswer: string | null; matchType: string | null; responseMs: number; points: number; hintsUsed?: number; rubricScore?: number | null; assessment?: SpeakingAssessment | null; scoreComponents?: { base?: number; accuracy?: number; difficulty?: number; speed?: number; streak?: number; hintDeduction?: number } | null };
type ReviewRound = { id: string; round: number; mode: string; prompt: string; instruction: string; timeLimit: number; publicData?: Record<string, unknown>; canonicalAnswer: string; acceptedAnswers: string[]; explanation: string; source?: { id: string; license_id: string; attribution: Record<string, unknown>; learning_sources?: { display_name?: string; homepage_url?: string; license_url?: string } | { display_name?: string; homepage_url?: string; license_url?: string }[] } | null; submissions: ReviewSubmission[] };
...
export function MatchReview({ data }: { data: MatchReviewData }) {
  ...
  return <main className={`${styles.root} app-shell`}><div className="app-container review-match-page">
    ...
    {data.rounds.some((round) => round.source) && <section className="surface review-attribution"><span className="eyebrow">OPEN DATA PROVENANCE</span><h2>Nguồn được dùng trực tiếp trong trận</h2><div>{[...new Map(data.rounds.flatMap((round) => round.source ? [[round.source.id, round.source] as const] : [])).values()].map((source) => { const details = sourceDetails(source); return <article key={source.id}><strong>{details.name}</strong><span>{source.license_id}</span>{details.homepage ? <a href={details.homepage} target="_blank" rel="noreferrer">Xem nguồn</a> : null}{details.license ? <a href={details.license} target="_blank" rel="noreferrer">Giấy phép</a> : null}</article>; })}</div></section>}
...
function sourceDetails(source: NonNullable<ReviewRound["source"]>) {
  const linked = Array.isArray(source.learning_sources) ? source.learning_sources[0] : source.learning_sources;
  return { name: linked?.display_name ?? String(source.attribution.source ?? "Nguồn mở đã duyệt"), homepage: linked?.homepage_url, license: linked?.license_url };
}
...
function SpeakingRubric({ assessment }: { assessment: SpeakingAssessment }) {
  ...
  const pronunciationDetail: [string, number | undefined][] = [["Dễ hiểu", assessment.intelligibility], ["Âm", assessment.segmental], ["Trọng âm", assessment.wordStress], ["Nhịp", assessment.rhythm], ["Ngữ điệu", assessment.intonation]];
  return <div className="review-rubric"><div>{scores.map(([label, score]) => <span key={label}><small>{label}</small><strong>{Math.round(score ?? 0)}</strong></span>)}</div>{assessment.intelligibility != null && <div>{pronunciationDetail.map(([label, score]) => <span key={label}><small>{label}</small><strong>{Math.round(score ?? 0)}</strong></span>)}</div>}{assessment.feedbackVi && <p>{assessment.feedbackVi}</p>}{assessment.phonemeFeedback?.map((item) => <p key={`${item.phoneme}:${item.example}`}><strong>/{item.phoneme}/</strong> {item.issue} · {item.example}</p>)}{assessment.practiceDrills?.length ? <p>Bài luyện: {assessment.practiceDrills.join(" · ")}</p> : null}</div>;
}
