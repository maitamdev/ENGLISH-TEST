...
import { requiresAudioReadiness } from "@/lib/learning/arena-adaptation";
...
export function RoomExperience({ initial }: { initial: RoomBootstrap }) {
  ...
  async function runAudioPreflight() {
    ...
    try {
      ...
      setPreflight(result);
      await fetch(`/api/rooms/${room.roomId}/readiness`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(result) });
    } catch (error) {
      const result: AudioPreflight = { status: "warning", microphone: false, inputLevel: 0, outputDevice: false, relayCandidate: false, turnConfigured: iceServersRef.current.length > 1, message: error instanceof Error ? error.message : "Không kiểm tra được thiết bị audio" };
      setPreflight(result);
      await fetch(`/api/rooms/${room.roomId}/readiness`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(result) }).catch(() => undefined);
    } finally {
      probe?.close();
    }
  }
    ...
    if (phase === "battle" && room.match?.question) return <div className="battle-shell">
      ...
      <section className="surface question-stage">
        <div className="question-stage-meta">
          <span className="mode-label">{room.match.question.mode.replaceAll("_", " ")}</span>{room.match.connectivityExtensionMs > 0 && <span className="fairness-extension">Đã bù {(room.match.connectivityExtensionMs / 1000).toFixed(1)}s do reconnect</span>}
