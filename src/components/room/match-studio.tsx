...
import {
  ...
  MessageCircleMore, Settings2, Sparkles, Swords, UsersRound
} from "lucide-react";
...
export function MatchStudio({ value, onChange, disabled }: Props) {
  ...
  return <section className="match-studio" aria-label="Tùy chỉnh trận đấu">
    ...
    <div className="studio-quick-settings">
      ...
      <label><span>Thời gian</span><select value={value.timePerQuestion ?? selected.timePerQuestion} onChange={(event) => onChange({ ...value, timePerQuestion: Number(event.target.value) })} disabled={disabled}>{[20,30,45,60,90,120].map((time) => <option value={time} key={time}>{time} giây</option>)}</select></label>
    ...
    {advancedOpen && <div className="studio-advanced">
      ...
      <fieldset><legend><AudioLines size={16} /> Thi nghe</legend><div className="inline-fields"><label><span>Giọng</span><select value={settings.listeningAccent} onChange={(event) => updateSettings({ listeningAccent: event.target.value as MatchSettings["listeningAccent"] })}><option value="US">Mỹ</option><option value="UK">Anh</option><option value="AU">Úc</option></select></label><label><span>Tốc độ</span><select value={settings.listeningSpeed} onChange={(event) => updateSettings({ listeningSpeed: Number(event.target.value) as MatchSettings["listeningSpeed"] })}><option value="0.75">0.75x</option><option value="1">1x</option><option value="1.25">1.25x</option></select></label><label><span>Số lần nghe</span><input type="number" min={1} max={5} value={settings.replayLimit} onChange={(event) => updateSettings({ replayLimit: Number(event.target.value) })} /></label></div></fieldset>
